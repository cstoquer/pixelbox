...
function getSettings() {
	...
	settings.components.pixelboxCore = settings.components.pixelboxCore === undefined ? true : settings.components.pixelboxCore;
	settings.components.AudioManager = settings.components.AudioManager === undefined ? true : settings.components.AudioManager;
	settings.components.TINA         = settings.components.TINA         === undefined ? true : settings.components.TINA;
	return settings;
}
...
function getCommandModules() {
	var fileList = fs.readdirSync(path.join(__dirname, 'commands'));

	for (var i = 0; i < fileList.length; i++) {
		var fileName = fileList[i];
		var moduleName = path.parse(fileName).name;
		addCommandModule(moduleName);
	}
}
