...
window.spritesheet = function(img) {
	return Texture.prototype.setSpritesheet(img);
};
...
function createScreen(width, height, pixelSize) {
	...
	style.width  = width  * pixelSize + 'px';
	style.height = height * pixelSize + 'px';
	return texture;
}

var screen = window.$screen = createScreen(SCREEN_WIDTH, SCREEN_HEIGHT, PIXEL_SIZE);
