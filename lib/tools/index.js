...
function MapLayerDropArea(parent, layer) {
	this.dom = createDiv('mapLayerDropArea', parent);
	this.layer = layer;
	dragManager.setAsDroppable(this.dom, this);
}
...
MapLayerDropArea.prototype.drop = function (id, item) {
	console.log('SET LAYER', this.layer, id, item);
	if (id === 'imageFile') {
		// TODO
	} else if (id === 'paletteColor') {
		// TODO
	}
};
...
function MapEditorPanel() {
	...
	dragManager.on('dragStart', function (id) {
		if (id !== 'imageFile' && id !== 'paletteColor') return;
	...
	// var backgroundDropArea = createDiv('mapLayerDropArea', dropAreas);
	// var foregroundDropArea = createDiv('mapLayerDropArea', dropAreas);
