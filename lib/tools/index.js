...
function MapEditorPanel() {
	...
	button(this.btnSave, function () { self.saveMap(); });
	...
	this.inputName = createDom('input', 'mapInput', this.settings);
	...
	button(okButton, function () {
		...
		if (w === self.map.width && h === self.map.height) return;
		self.resize(w, h);
	});
	...
	button(btnSettings, toggleSettingDisplay);
...
function MapListItem(index, map, parent) {
	...
	createDom('spawn', 'mapListItemIndex', this.dom).innerText = index;
	// TODO untitled in italic
	createDom('spawn', 'mapListItemName',  this.dom).innerText = map.name || 'untitled';
