...
function MapPanel() {
	...
	this.clipSurface = createDiv('mapClipSurface', this.content);
	var grid = createDiv('mapGrid', this.clipSurface);
	...
	this.clipSurface.appendChild(canvas);

	button(grid, function (e) {
		// console.log(e)
		var x = ~~(e.layerX / 24);
		var y = ~~(e.layerY / 24);
		self.map.addItem(x, y, spritesheet.sprite, spritesheet.flipH, spritesheet.flipV, spritesheet.flipR);
	});
	...
	function startDrag(e) {
		...
		function dragMove(e) {
			e.preventDefault();
			posX = clip(e.clientX - startX, -(self.map.width  * 8 - 128) * PIXEL_SIZE - 1, 0);
			posY = clip(e.clientY - startY, -(self.map.height * 8 - 128) * PIXEL_SIZE - 1, 0);
